...
function Server(opts) {
    var self = this;
    Connection.Connection.call(this, opts);

    this.xmlns[''] = NS_SERVER;
    this.xmlns['db'] = NS_DIALBACK;
    this.streamAttrs.version = '1.0';
...
exports.IncomingServer = function(stream, credentials) {
    ...
    this.startParser();
    this.streamAttrs.id = generateId();

    this.addListener('streamStart', function(streamAttrs) {
        if (streamAttrs.to && credentials && credentials.hasOwnProperty(streamAttrs.to))
        ...
        if (!self.credentials)
            delete self.streamAttrs.version;
...
exports.IncomingServer.prototype.startStream = function() {
    ...
    if (this.streamAttrs.version == '1.0') {
...
exports.OutgoingServer = function(srcDomain, destDomain, credentials, opts) {
    var self = this;
    ...
    // side. Unfortunately this is required for XMPP 1.0.
    if (!this.credentials)
        delete this.streamAttrs.version;
    ...
    this.addListener('streamStart', function(attrs) {
        if (attrs.version !== "1.0")
            ...
            var authAttrs = {method: 'dialback', uid: attrs.id};
            self.emit('auth', authAttrs);
    });
    self.addListener('stanza', function(stanza) {
