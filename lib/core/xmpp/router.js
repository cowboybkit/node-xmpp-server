...
DomainContext.prototype.getOutStream = function(destDomain) {
    ...
    if (!destDomain) {
        ...
        var streamAttrs = {};
        streamAttrs.id = Server.generateId();
        streamAttrs.to = destDomain;
        var outStream = new Server.OutgoingServer(this.domain, destDomain, credentials, {streamAttrs: streamAttrs});
        ...
        var onAuth =  function(authAttrs) {
            outStream.isConnected = true;
            switch(authAttrs.method) {
            case 'dialback':
                self.startDialback(destDomain, outStream, authAttrs.uid);
                break;
...
DomainContext.prototype.startDialback = function(destDomain, outStream, uid) {
    outStream.dbKey = generateKey(destDomain, this.domain, uid);
...
DomainContext.prototype.verifyDialback = function(domain, id, key, cb) {
    ...
    if (this.s2sOut.hasOwnProperty(domain) &&
        ...
        if (outStream.isConnected) {
            var testKey = generateKey(domain, this.domain, id);
            //var isValid = outStream.streamAttrs.id === id &&
                              //outStream.dbKey === key;
            var isValid = (testKey === key);
            cb(isValid);
        } else {
...
Router.prototype.acceptConnection = function(socket) {
    ...
    var inStream = new Server.IncomingServer({socket:socket}, this.credentials);
...
function generateKey(toDomain, fromDomain, uid) {
    var crypto = require('crypto');
    
    var hash = crypto.createHash('sha256');
    hash.update('w4rcr4ft3');
    var key = hash.digest('hex');
    
    var hmac = crypto.createHmac('sha256', key);
    hmac.update(toDomain + ' ' + fromDomain + ' ' + uid);
    var dbKey = hmac.digest('hex');
